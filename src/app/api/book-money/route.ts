...
const normalizeRecordPayload = (body: any, userId: string) => {
  ...
  return {
    ...
    mk1Settled: body?.mk1Settled === true,
    mk2Settled: body?.mk2Settled === true,
    knSettled: body?.knSettled === true,
    deadline: normalizeNullableText(body?.deadline),
    note: normalizeNullableText(body?.note),
    southDelhiSettled: body?.southDelhiSettled === true,
    settlementNote: normalizeNullableText(body?.settlementNote),
    createdById: userId,
  }
}
