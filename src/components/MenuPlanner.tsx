        ...
        >
          <div className="mb-4">
            ...
            <ArrayInput
              ...
              className={`app-input ${isRenderingExport ? 'text-4xl leading-relaxed min-h-[160px]' : ''}`}
          ...
          <div id="menu-table-container" className="overflow-x-auto bg-[var(--surface)] p-2 -mx-2 rounded-xl">
            <table className={`w-full table-auto border-collapse border border-[var(--border)] min-w-[1200px] ${isRenderingExport ? 'text-4xl' : ''}`}>
            <thead>
              <tr className="bg-[var(--surface-soft)]">
                <th className={`border border-[var(--border)] ${isRenderingExport ? 'p-10 text-4xl' : 'p-2'}`}>Day</th>
                <th className={`border border-[var(--border)] ${isRenderingExport ? 'p-10 text-4xl' : 'p-2'}`}>Lunch</th>
                <th className={`border border-[var(--border)] ${isRenderingExport ? 'p-10 text-4xl' : 'p-2'}`}>Cooking Team</th>
                <th className={`border border-[var(--border)] ${isRenderingExport ? 'p-10 text-4xl' : 'p-2'}`}>Dinner</th>
                <th className={`border border-[var(--border)] ${isRenderingExport ? 'p-10 text-4xl' : 'p-2'}`}>Dinner Cooking Team</th>
              </tr>
            </thead>
            <tbody>
              {(menu.items || []).map((item, index) => (
                <tr key={item.day} className="border-b border-[var(--border)]">
                  <td className={`border border-[var(--border)] font-medium align-top ${isRenderingExport ? 'p-10 text-4xl' : 'p-2'}`}>
                    {item.day === 'Sunday' ? "Lord's Day" : item.day}
                  </td>
                  <td className={`border border-[var(--border)] ${isRenderingExport ? 'p-10' : 'p-2'}`}>
                    <textarea
                      value={item.lunch || ''}
                      onChange={(e) => updateMenuItem(index, 'lunch', e.target.value)}
                      className={`app-input resize-y w-full ${isRenderingExport ? 'min-h-[220px] text-3xl leading-relaxed' : 'min-h-[80px]'}`}
                  ...
                  <td className={`border border-[var(--border)] ${isRenderingExport ? 'p-10' : 'p-2'}`}>
                    <ArrayInput
                      ...
                      className={`app-input ${isRenderingExport ? 'text-3xl leading-relaxed min-h-[220px]' : ''}`}
